...
fn connect(
    ...
    use mio::sys::unix::fd::Socket;
    let socket = Socket::new(addr, 1)
        .map_err(ConnectError::m("socket creation failed"))?;
    match config.local_address_ipv4 {
        Some(ipv4) => {
            socket.bind_local_address(Some(SocketAddr::new(IpAddr::from(ipv4), 0)), None)
                .map_err(ConnectError::m("tcp bind local error"))?;
        }
        None => {}
    }

    match config.local_address_ipv6 {
        Some(ipv6) => {
            socket.bind_local_address(Some(SocketAddr::new(IpAddr::from(ipv6), 0)), None)
                .map_err(ConnectError::m("tcp bind local error"))?;
        }
        None => {}
    }

    let connect = mio::sys::TcpStream::connect_from_socket(socket, addr.clone()).map_err(ConnectError::m("connect_from_socket tcp connect error"))?;
